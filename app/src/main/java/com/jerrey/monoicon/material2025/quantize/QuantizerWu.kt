 ...
 */
package com.jerrey.monoicon.material2025.quantize

import com.jerrey.monoicon.material2025.utils.ColorUtils
...
class QuantizerWu : Quantizer {
  // Long is required for full-resolution wallpapers: RGB moments exceed Int.MAX_VALUE.
  private lateinit var weights: LongArray
  private lateinit var momentsR: LongArray
  private lateinit var momentsG: LongArray
  private lateinit var momentsB: LongArray
  ...
  fun constructHistogram(pixels: Map<Int, Int>) {
    weights = LongArray(TOTAL_SIZE)
    momentsR = LongArray(TOTAL_SIZE)
    momentsG = LongArray(TOTAL_SIZE)
    momentsB = LongArray(TOTAL_SIZE)
  ...
  fun createMoments() {
    for (r in 1 until INDEX_COUNT) {
      val area = LongArray(INDEX_COUNT)
      val areaR = LongArray(INDEX_COUNT)
      val areaG = LongArray(INDEX_COUNT)
      val areaB = LongArray(INDEX_COUNT)
  ...
  fun createResult(colorCount: Int): List<Int> {
    ...
    for (i in 0 until colorCount) {
      ...
      if (weight > 0) {
        val r = (volume(cube, momentsR) / weight).toInt()
        val g = (volume(cube, momentsG) / weight).toInt()
        val b = (volume(cube, momentsB) / weight).toInt()
  ...
  internal fun variance(cube: Box): Double {
    val dr = volume(cube, momentsR).toDouble()
    val dg = volume(cube, momentsG).toDouble()
    val db = volume(cube, momentsB).toDouble()
    val xx =
  ...
  internal fun maximize(
    ...
    for (i in first until last) {
      ...
      var tempNumerator = halfR.toDouble() * halfR + halfG.toDouble() * halfG + halfB.toDouble() * halfB
      ...
      tempNumerator = halfR.toDouble() * halfR + halfG.toDouble() * halfG + halfB.toDouble() * halfB
  ...
  companion object {
    ...
    internal fun volume(cube: Box, moment: LongArray): Long {
    ...
    internal fun bottom(cube: Box, direction: Direction, moment: LongArray): Long {
    ...
    internal fun top(cube: Box, direction: Direction, position: Int, moment: LongArray): Long {
      return when (direction) {
        Direction.RED ->
