...
//
// Ported from the Miuix example's `component/liquid/Lens.kt`
// (https://github.com/compose-miuix-ui/miuix), which is itself adapted from
// Kyant0/AndroidLiquidGlass (Apache 2.0). `vibrancy()`/`lens()` are NOT part of
// the published miuix-blur API — they are example-side code built on the public
// `BackdropEffectScope.runtimeShaderEffect`, so they are vendored here.
//
// Only the Compose-internal helpers the original used (`fastCoerceAtMost`) were
// replaced with their public equivalents; the shaders are unchanged.
 ...
 *
 * @param chromaticAberration Strength of the rim chromatic dispersion. `0`
 *  disables the effect (cheaper non-dispersion shader is used). Typical values:
 *  `0.1` for subtle, `0.2` for Apple-pill-like, `0.3+` for pronounced rainbow
 *  halo. The dispersion offset scales with the refraction depth so it
 *  concentrates at the rim band's outer edge.
 */
fun BackdropEffectScope.lens(
    ...
    runtimeShaderEffect(
        ...
        setFloatUniform("cornerRadius", scaledRadii)
...
half4 main(float2 coord) {
    ...
    float radius = radiusAt(centeredCoord, cornerRadius);
