...
//
// Ported from the Miuix example's `component/liquid/CombinedBackdrop.kt`.
// Example-side helper, not part of the published miuix-blur API.
...
/**
 * A [Backdrop] that draws [first] then [second] in order, allowing a
 * tinted/overlay backdrop to be sampled on top of a base backdrop. Mirrors
 * Kyant's `CombinedBackdrop` pattern used in `LiquidBottomTabs` to layer a
 * recorded "tinted tabs" pass over the underlying app background as a single
 * sampling source for an indicator.
 */
@Stable
